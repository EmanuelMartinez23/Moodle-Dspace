...
// funcional
function create_or_get_block_instance($blockname, $contextid) {
    // [1]
    global $DB;

    // [2]    
    // arreglo de parametros para la consulta
    $params = ['blockname' => $blockname, 'parentcontextid' => $contextid];
    
    // [3]
    // hacemos la query a la table de block_instances con los parametros
    $block_instance = $DB->get_record('block_instances', $params);

    // [4]
    // si no tennemos una instancia en ese tabla la creamos
    if (!$block_instance) {
        $block_instance = new stdClass();
        $block_instance->blockname = $blockname;
        $block_instance->parentcontextid = $contextid;
        $block_instance->showinsubcontexts = 0;
        $block_instance->requiredbytheme = 0;
        $block_instance->pagetypepattern = 'mod-assignment-*';
        $block_instance->defaultregion = 'content';
        $block_instance->defaultweight = 0;
        $block_instance->timecreated = time();
        $block_instance->timemodified = time();

        // [5]
        // insertamos la instancia en la DB
...
function update_itemid_for_introattachments() {
    global $DB;
    $DB->execute("UPDATE {files} SET itemid = 0 WHERE filearea = 'introattachment'");
}
